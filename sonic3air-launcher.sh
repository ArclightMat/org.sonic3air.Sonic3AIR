...
function install_game () {
    ...
    if [ $? -eq 1 ]; then
        ...
        if [ "$2" -eq 1 ]; then
            ...
            # TODO: Use the following line when Zenity gets updated to 4.x in FDO.
            # zenity --error --width 200 --height 100 --icon="${eggman_icon}" --title="${error_title}" --text="${error_text}"
...
function detect_game () {
    ...
    # TODO: Use the following line when Zenity gets updated to 4.x in FDO.
    # zenity --question --width 600 --height 300 --icon"${eggman_icon}" --title="${not_found_title}" --text="${not_found_text}"
